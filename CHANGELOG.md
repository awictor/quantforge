...
### Documentation
- README: documented `ackermann` in the LQR section after the Lyapunov note -- the closed-form
  pole-placement gain, the exact eigenvalue assignment, and when to prefer it over LQR. All
...
### Documentation
- README: documented `gmres` in the iterative-solvers section after `conjugate_gradient` -- the
  Arnoldi/residual-minimizing scheme, the restart and matvec-callable options, and when to prefer
  it over CG. All snippet values verified live.
...
### Documentation
- README: documented `isomap` and `knn_graph` in the PCA section after `classical_mds` -- the
  geodesic-distance approach to curved manifolds, the arc-unrolling behaviour, and the
  `n_neighbors` tradeoff. All snippet values verified live.
...
### Documentation
- README: documented `cp_als` and `cp_reconstruct` in the Matrix utilities section after `cur` --
  the tensor CP decomposition, the ALS/Khatri-Rao fit, and the uniqueness that makes its factors
  interpretable. All snippet values verified live.
...
### Documentation
- README: documented `nmf` in the Matrix utilities section after `randomized_svd` -- the
  Lee-Seung multiplicative updates, the parts-based interpretability, and when to prefer it over
  SVD/PCA. All snippet values verified live.
...
### Documentation
- README: documented `matrix_pencil` in the spectral-analysis section after the Prony note --
  the Hankel-shift SVD estimator, the noise-subspace rejection, and when to prefer it over
  `prony`. All snippet values verified live.
...
### Documentation
- README: documented `aaa` in the interpolation block after the Floater-Hormann note -- the
  adaptive greedy support-point selection, the Loewner-SVD weights, pole capture, and the
  contrast with fixed-node rational interpolation. All snippet values verified live.
...
### Documentation
- README: documented `GKQuantile` in the streaming-statistics section after `KLL` -- the
  deterministic rank guarantee, the ``(value, g, delta)`` tuples, and a decision guide across
  KLL / GKQuantile / TDigest / DDSketch. All snippet values verified live.
...
### Documentation
- README: documented `KLL` in the streaming-statistics section after `TDigest` -- the compactor
  hierarchy, the provable rank-error bound, and the rank-vs-value-error distinction. All snippet
...
### Documentation
- README: documented `bayesian_optimize` and `expected_improvement` in the Gaussian process
  regression section -- the GP-surrogate loop, the EI acquisition, and the evaluation savings.
  All snippet values verified live.
...
### Documentation
- README: documented `gelman_rubin`, `integrated_autocorrelation_time`, and
  `effective_sample_size` in the Markov chain Monte Carlo section -- the R-hat convergence check,
  the autocorrelation-time / ESS pair, and how to read them. All snippet values verified live.
...
### Documentation
- README: documented `sliced_wasserstein` in the Wasserstein section after the Sinkhorn note --
  the random-projection scheme, the cost advantage, the translation closed form, and
  Sinkhorn-vs-sliced guidance. All snippet values verified live.
...
### Documentation
- README: documented `unscented_kalman_filter` in the Kalman-filter section after the EKF note
  -- sigma-point propagation, the van der Merwe scaling, the linear-case match, and EKF-vs-UKF
  guidance. All snippet values verified live.
...
### Documentation
- README: documented `newton_min` in the optimization block after `lbfgs` -- the autodiff
  gradient/Hessian, the Levenberg-damped Newton step, and when to prefer it over `lbfgs`. All
  snippet values verified live.
...
### Documentation
- README: documented `spherical_bessel_j`/`spherical_bessel_y` and `bessel_in`/`bessel_kn` in
  the special-functions block after the orthogonal-polynomial note -- the recurrence strategy
  per function and the series/integral/Wronskian cross-checks. All snippet values verified live.
...
### Documentation
- README: documented `riemann_zeta` and `dirichlet_eta` in the special-functions block after
  the Airy functions -- the Euler-Maclaurin evaluation, the eta-relation continuation on the
  critical strip, and Cohen-Villegas-Zagier acceleration. All snippet values verified live.
...
### Documentation
- README: documented `airy_ai` and `airy_bi` in the special-functions block after the
  Fresnel integrals -- the Airy functions solving ``y'' = x y``. All snippet values verified
  live.
...
### Documentation
- README: documented `bessel_j0`, `bessel_j1`, `bessel_jn`, `bessel_y0`, and `bessel_y1` in
  the special-functions block after the elliptic integrals -- Bessel functions of the first
  and second kind. All snippet values verified live.
...
### Documentation
- README: documented `voronoi_vertices`, `delaunay_neighbors`, and `nearest_site` in the
  Computational geometry section after `delaunay_triangulation` -- the Voronoi diagram as the
  Delaunay dual. All snippet values verified live.
...
### Documentation
- README: documented `angular_distance`, `slerp_vectors`, `spherical_centroid`, and
  `spherical_resultant_length` in the Quaternions section after the SO(3) map -- unit-sphere
  angular geometry. All snippet values verified live.
...
### Documentation
- README: documented `quat_to_matrix`/`matrix_to_quat`/`euler_to_quat`/`quat_to_euler` (and
  the matrix/Euler shortcuts) in the Quaternions section -- conversions among quaternion,
  rotation matrix, and Z-Y-X Euler angles. All snippet values verified live.
...
### Documentation
- README: documented `fd_weights` in the Numerical utilities section after the Ridders
  derivatives -- Fornberg finite-difference stencil weights for any order on an arbitrary
  grid. All snippet values verified live.
...
### Documentation
- README: documented `RunningMedian` in the streaming-statistics section after `P2Quantile`
  -- the exact two-heap running median versus the constant-memory P2 estimate. All snippet
  values verified live.
...
### Documentation
- README: documented `LazySegmentTree` in the Range-query structures section after
  `SegmentTree` -- range-add updates with range sum/min/max queries via lazy propagation.
  All snippet values verified live.
...
### Documentation
- README: documented `floyd_cycle`, `brent_cycle`, and `cycle_elements` in the Numerical
  utilities section after the Sturm block -- constant-memory cycle detection for iterated
  maps. All snippet values verified live.
...
### Documentation
- README: documented `elementary_symmetric`, `power_sums`, `poly_from_roots`,
  `power_to_elementary`, and `elementary_to_power` in the Number theory section after the
  combinatorics block -- symmetric functions and Newton's identities. All snippet values
  verified live.
...
### Documentation
- README: documented `linear_recurrence_nth`, `matrix_power`, and `fibonacci` in the Number
  theory section after `XorBasis` -- companion-matrix exponentiation for the ``n``-th term
  of a linear recurrence in ``O(log n)``. All snippet values verified live.
...
### Documentation
- README: documented `weighted_interval_schedule`, `activity_selection`, and `min_rooms` in
  the Range-query structures section after the interval-set operations -- max-weight DP,
  earliest-finish greedy, and the endpoint-sweep room count. All snippet values verified
  live.
...
### Documentation
- README: documented `dft_any`/`idft_any` in the Spectral analysis section after the
  radix-2 `fft`/`ifft` -- the arbitrary-length Bluestein chirp-Z transform and its
  agreement with `fft` on power-of-two lengths. All snippet values verified live.
...
### Documentation
- README: documented `Trie` in the String algorithms section after the suffix-array
  routines -- prefix-tree membership, `starts_with`/`count_prefix`/`keys_with_prefix`, and
  `longest_prefix_of`. All snippet values verified live.
...
### Documentation
- README: documented `LCA` in the Graph algorithms section after the shortest-path
  routines -- binary-lifting `query`, `depth`/`distance`/`is_ancestor`, and the iterative
  build. All snippet values verified live.
...
### Documentation
- README: documented `SparseTable` in the Range-query structures section after
  `FenwickTree`/`SegmentTree` -- `O(1)` idempotent range min/max/gcd on a static array, the
  power-of-two block precomputation, and why overlap restricts it to idempotent combiners.
  All snippet values verified live.
