...
def bermudan_basket_lsm(S1, S2, w1, w2, K, t, r, sigma1, sigma2, rho,
    ...
    def payoff(a, bb):
        return max(sign * (w1 * a + w2 * bb - K), 0.0)
    ...
    for step in range(n_steps - 1, 0, -1):
        ...
        if len(itm) > 8:
            ...
            for p in itm:
                ...
                bk = w1 * a + w2 * bb
                X.append([1.0, a, bb, a * a, bb * bb, a * bb, bk])
                Y.append(cash[p] * disc)
            beta = _solve_normal_equations(X, Y)
            for p in itm:
                a, bb = p1[p][step], p2[p][step]
                bk = w1 * a + w2 * bb
                basis = [1.0, a, bb, a * a, bb * bb, a * bb, bk]
...
def bermudan_spread_lsm(S1, S2, K, t, r, sigma1, sigma2, rho,
    ...
    for step in range(n_steps - 1, 0, -1):
        ...
        if len(itm) > 8:
            ...
            for p in itm:
                a, bb = p1[p][step], p2[p][step]
                X.append([1.0, a, bb, a * a, bb * bb, a * bb, a - bb])
                Y.append(cash[p] * disc)
            beta = _solve_normal_equations(X, Y)
            for p in itm:
                a, bb = p1[p][step], p2[p][step]
                basis = [1.0, a, bb, a * a, bb * bb, a * bb, a - bb]
                cont = sum(beta[j] * basis[j] for j in range(len(basis)))
                ex = payoff(a, bb)
                cash[p] = ex if ex >= cont else cash[p] * disc
            for p in range(n_paths):
...
def bermudan_spread_lsm_greeks(S1, S2, K, t, r, sigma1, sigma2, rho,
                               q1=0.0, q2=0.0, option_type=OptionType.CALL,
    ...
    def px(s1, s2):
        return bermudan_spread_lsm(s1, s2, K, t, r, sigma1, sigma2, rho,
...
def bermudan_min_put_lsm_greeks(S1, S2, K, t, r, sigma1, sigma2, rho,
                                q1=0.0, q2=0.0, n_steps=50, n_paths=40_000,
                                seed=None, h_rel=0.01):
    ...
    def px(s1, s2):
        return bermudan_min_put_lsm(s1, s2, K, t, r, sigma1, sigma2, rho,
                                    q1, q2, n_steps=n_steps, n_paths=n_paths,
                                    seed=seed)
