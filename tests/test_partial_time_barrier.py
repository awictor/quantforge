"""Partial-time (end) single-barrier call (Heynen-Kat 1994)."""

import math

import pytest

from quantforge import partial_time_end_barrier_call, barrier_option
...
def test_limit_full_monitoring_approaches_standard_barrier():
    # t1 -> 0: barrier is live essentially the whole life -> standard down-out.
    pt = partial_time_end_barrier_call(S, K, H, 1e-4, T, R, SIG, "down-out")
...
def test_limit_no_monitoring_approaches_vanilla():
    # t1 -> T2: barrier barely watched -> vanilla call.
    pt = partial_time_end_barrier_call(S, K, H, T - 1e-4, T, R, SIG, "down-out")
    assert pt == pytest.approx(call_price(S, K, T, R, SIG), abs=1e-2)


def test_between_standard_and_vanilla():
    pt = partial_time_end_barrier_call(S, K, H, 0.5, T, R, SIG, "down-out")
    full = barrier_option(S, K, H, T, R, SIG, "call", "down-out")
...
def test_in_out_parity():
    ki = partial_time_end_barrier_call(S, K, H, 0.5, T, R, SIG, "down-in")
    ko = partial_time_end_barrier_call(S, K, H, 0.5, T, R, SIG, "down-out")
...
def test_validation():
    with pytest.raises(ValueError):
        partial_time_end_barrier_call(S, K, H, 0.0, T, R, SIG, "down-out")  # t1=0
    with pytest.raises(ValueError):
        partial_time_end_barrier_call(S, K, H, 1.5, T, R, SIG, "down-out")  # t1>T2
    with pytest.raises(ValueError):
        partial_time_end_barrier_call(S, K, 130.0, 0.5, T, R, SIG, "up-out")  # unsupported
